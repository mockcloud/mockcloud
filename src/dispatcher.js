...
const SQS_ACTIONS = new Set(['CreateQueue','GetQueueUrl','ListQueues','DeleteQueue','SendMessage','SendMessageBatch','ReceiveMessage','DeleteMessage','DeleteMessageBatch','ChangeMessageVisibility','ChangeMessageVisibilityBatch','GetQueueAttributes','SetQueueAttributes','PurgeQueue']);
...
const SES_ACTIONS = new Set(['SendEmail','SendRawEmail','VerifyEmailIdentity','VerifyEmailAddress','ListIdentities','ListVerifiedEmailAddresses','DeleteIdentity','GetSendQuota','GetSendStatistics','GetIdentityVerificationAttributes']);
...
export function dispatchAWS(req, res) {
  ...
  if (target.startsWith('AmazonSimpleEmailService') || SES_ACTIONS.has(action)) return sesHandler(req, res);
